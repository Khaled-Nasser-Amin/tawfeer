...
class LogoutOtherBrowserSessionsForm extends Component
{
    ...
    public function logoutOtherBrowserSessions()
    {
        ...
        \auth()->guard('vendor')->user()->forceFill([
            'remember_token' => null
        ]);
        \auth()->guard('vendor')->user()->save();

        auth()->guard('vendor')->logoutOtherDevices($this->password);
    ...
    protected function deleteOtherSessionRecords()
    {
        ...
        DB::connection(config('session.connection'))->table(config('session.table', 'sessions'))
            ->where('vendor_id', Auth::guard('vendor')->user()->getAuthIdentifier())
    ...
    {
        return view('front.Profile.logout-other-browser-sessions-form');
    }
}

@php
$banners= \App\Models\Banner::where('show_in','home')->where(function($q){
    return $q->where('expire_at','>',Carbon\Carbon::now()->toDateTimeString())->orWhere('expire_at',null);
});
@endphp
<div class="wrap-main-slide my-4 {{ $banners->count() > 0 ? '' : 'd-none' }}" style="height: 400px">
        ...
        <div class="carousel-inner h-100">
            @foreach($banners->get() as $banner)
                <div class="carousel-item {{$loop->index == 0 ? 'active' : ''}} img-slide w-100  h-100" style="background-image: url('{{$banner->image}}');background-size: cover;background-repeat: no-repeat">
                   @if ($banner->url)
                        <a class="btn  banner_url  d-flex justify-content-center align-items-center" href="{{$banner->url}}" >@lang('text.View')</a>
...
@if(\App\Models\Category::count()>0)
    <div class="wrap-banner style-twin-default my-3">
